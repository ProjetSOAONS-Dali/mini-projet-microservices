...
const getNotifications = async (call, callback) => {
  ...
  } catch (e) { callback({ code: grpc.status.INTERNAL, message: e.message }); }
};
...
const markAsRead = async (call, callback) => {
  try {
    ...
    const doc = await db.notifications
      .findOne(call.request.notification_id).exec();
    if (!doc)
...
const start = async () => {
  await startConsumer(); // Kafka Consumer démarre en arrière-plan
  ...
  server.bindAsync(`0.0.0.0:${PORT}`, grpc.ServerCredentials.createInsecure(), (err) => {
    if (err) { console.error('❌ NotificationService:', err); return; }
    console.log(`✅ NotificationService gRPC démarré → port ${PORT}`);
  });
};
